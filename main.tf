###sqs queue with existing dead-letter-queue attach code
...
resource "aws_sqs_queue" "queue" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.deadletter_queue.arn,#########(existing dead-letter-queue-arn)
    maxReceiveCount     = 10
  })
  tags = var.tags
}
...
##################################################################
(aws user and IAM policy jasoncode as variable"policy_queue")
###################################################################
...
######################################################
create a sqs-queue with new dlq-attach policy
######################################################
...
##################################################################
(aws user and IAM policy jasoncode as variable"deadletter_queue")
###################################################################
